...
public class GameObjectManager {
    ...
    public Keyword getKeyword() {
        return (Keyword) this.vector
                .stream()
                .filter(gameObject -> gameObject instanceof Keyword) //duyet tat ca phan tu sao cho thoa dien kien truyen (thuc chat true || false) vao thi lay
...
// 1 số hàm khác hay gặp nữa đó là
// list.stream().map(interger -> interger *2 ).forEach(System.out::println);
// list.stream().map(interger -> String.format("%d", interger))
